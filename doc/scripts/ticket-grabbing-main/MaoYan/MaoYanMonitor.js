...
function doSubmit(amount, viewers) {
    ...
    //等待立即支付按钮出现（加超时，避免无 Button 时无限阻塞）
    var btnAppeared = className("android.widget.Button").waitFor(3000);
    if (!btnAppeared) {
        console.log("✗ 3秒内未出现支付按钮，可能页面未跳转或票档失效");
        return false;
    }
    //选择对应的观演人
...
// 旧逻辑用 className('android.widget.Button') 每 50ms 盲点，会点到任意按钮（含返回/取消），
// 且无库存检测；500+ 次点击 ≈ 25 秒，远超热门票库存预留窗口（5–10 秒），订单必然回库。
function handlePaymentPage() {
    var PAY_BUTTON_TEXTS = ["立即支付", "确认支付", "去支付"];
    var STOCK_OUT_KEYWORDS = ["库存不足", "已售罄", "票已售完", "已售完", "暂无余票", "无票", "票已经卖完"];
    var PAY_TIMEOUT_MS = 5000;
    var PAY_CLICK_MAX = 2;
